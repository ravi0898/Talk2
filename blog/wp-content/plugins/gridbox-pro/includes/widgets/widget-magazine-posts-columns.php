...
class Gridbox_Pro_Magazine_Columns_Widget extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct(
			...
			array(
				...
				'description' => esc_html__( 'Displays your posts from two selected categories.', 'gridbox-pro' ),
	...
	private function default_settings() {
		...
		$defaults = array(
			'category_one'       => 0,
			'category_two'       => 0,
			'category_one_title' => esc_html__( 'Left Category', 'gridbox-pro' ),
			'category_two_title' => esc_html__( 'Right Category', 'gridbox-pro' ),
			'number'             => 4,
			'highlight_post'     => true,
		);
	...
	function widget( $args, $instance ) {
		...
		if ( ! function_exists( 'gridbox_get_magazine_post_ids' ) ) {
	...
	function category_title( $args, $settings, $category_id, $category_title ) {
		...
		if ( ! empty( $widget_title ) ) :
			...
			$widget_title = gridbox_magazine_widget_title( $widget_title, $category_id );
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'category_one' ); ?>"><?php esc_html_e( 'Left Category:', 'gridbox-pro' ); ?></label><br/>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'category_two' ); ?>"><?php esc_html_e( 'Right Category:', 'gridbox-pro' ); ?></label><br/>
			<?php // Display Category One Select.
				$args = array(
					'show_option_all'    => esc_html__( 'All Categories', 'gridbox-pro' ),
					'show_count' 		 => true,
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'highlight_post' ); ?>">
				...
				<?php esc_html_e( 'Highlight first post (big image + excerpt)', 'gridbox-pro' ); ?>
			</label>
		</p>
