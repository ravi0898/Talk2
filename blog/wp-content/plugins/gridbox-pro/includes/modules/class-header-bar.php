...
class Gridbox_Pro_Header_Bar {
		...
		if ( has_nav_menu( 'social' ) or has_nav_menu( 'secondary' ) ) {
			...
			if ( has_nav_menu( 'secondary' ) ) : ?>
				...
				<button class="secondary-menu-toggle menu-toggle" aria-controls="secondary-menu" aria-expanded="false" <?php self::amp_menu_toggle(); ?>>
					...
					?>
					<span class="menu-toggle-text screen-reader-text"><?php esc_html_e( 'Menu', 'gridbox-pro' ); ?></span>
				</button>

				<div class="secondary-navigation">

					<nav class="top-navigation" role="navigation" <?php self::amp_menu_is_toggled(); ?> aria-label="<?php esc_attr_e( 'Secondary Menu', 'gridbox-pro' ); ?>">
	...
	static function nav_menu_social_icons( $item_output, $item, $depth, $args ) {
		...
		// Get SVG.
		$svg = apply_filters( 'gridbox_pro_get_social_svg', self::get_social_svg( $icon ), $item_output );
	...
	static function get_social_svg( $icon = null ) {
		...
		$svg .= ' <use xlink:href="' . GRIDBOX_PRO_PLUGIN_URL . 'assets/icons/social-icons.svg#icon-' . esc_html( $icon ) . '"></use> ';
		$svg .= '</svg>';
	...
	static function supported_social_icons() {
		...
		return apply_filters( 'gridbox_pro_supported_social_icons', $supported_social_icons );
	}
