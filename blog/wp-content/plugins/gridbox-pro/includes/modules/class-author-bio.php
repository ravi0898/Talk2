...
class Gridbox_Pro_Author_Bio {
	...
	static function setup() {
		...
		add_action( 'gridbox_author_bio', array( __CLASS__, 'author_bio' ) );
		...
		add_filter( 'gridbox_hide_elements', array( __CLASS__, 'hide_author_bio' ) );
	}
		...
		if ( true === $theme_options['author_bio'] || is_customize_preview() ) : ?>
			...
			<div class="entry-author clearfix">
				...
				<div class="author-heading">
					<h4 class="author-title"><?php printf( esc_html__( 'About %s', 'gridbox-pro' ), '<span class="author-name">' . get_the_author() . '</span>' ); ?></h4>
					<a class="author-link" href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ); ?>" rel="author">
						<?php esc_html_e( 'View all posts', 'gridbox-pro' ); ?>
					</a>
				</div>
	...
	static function author_bio_settings( $wp_customize ) {
		...
		$wp_customize->add_setting( 'gridbox_theme_options[author_bio]', array(
			'default'           => false,
			'type'              => 'option',
...
// Run Class.
add_action( 'init', array( 'Gridbox_Pro_Author_Bio', 'setup' ) );
